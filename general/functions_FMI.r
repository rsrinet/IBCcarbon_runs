...
runModel <- function(sampleID, outType="dTabs", uncRCP=0,
  ...
  if(rcpfile=="CurrClim"){
    ...
    dat[,rday:=xday]
      ...
      datname <- load(paste0(climatepath, fmi_vars_PREBAS_file))#"fmi_vars_PREBAS.rdata"))
      assign("dat",get(datname))
      lookupname <- load(paste0(climatepath, climID_lookup_file))#"climID_lookup.rdata"))
  ...
  if(rcpfile=="CurrClim_fmi") data.sample$id <- data.sample$climID <- 
    data.sample$CurrClimID <- lookup$climID[((sampleID-1)*nSample+1):(sampleID*nSample)]
  ...
  if(rcpfile%in%c("CurrClim","CurrClim_fmi") | climScen >0) {
    ...
    if(vPREBAS=="newVersion"){
      p0.fT0 <- calc.P0.fT0(dat, data.sample, startingYear, nYears)
      p0currClim <- p0.fT0$P0.dt
      fT0AvgCurrClim <- p0.fT0$fT0.dt
    }
    rm("dat")
  }
...
create_prebas_input.f = function(r_no, clim, data.sample, nYears,
  ...
  areas <- data.sample$area
  if (vPREBAS == "master") latitude=data.sample$lat
  if (vPREBAS == "newVersion" & rcps == "CurrClim_fmi"){
    latitude <- data.sample$lat
    P0currClim <- p0.fT0$P0.dt$P0currClim[unique(match(sampleX$climID,p0.fT0$P0.dt$id))]
    fT0AvgCurrClim <- p0.fT0$fT0.dt$fT0[unique(match(sampleX$id,p0.fT0$fT0.dt$id))]
  ...
  save(ClCuts,ftTapioParX,defaultThin,mortMod,energyCut,tTapioParX, file="thin_pars.rdata")
  if (vPREBAS == "master"){
...
prep.climate.f = function(dat, data.sample, startingYear, nYears, rcps = "CurrClim"){
  ...
  if(rcps == "CurrClim_fmi"){
    ...
    dat[, pvm:= as.Date('2015-01-01') - 1 + rday ]
...
calc.P0.fT0 <- function(dat, data.sample, startingYear, nYears){
smoothP0=0
...
for(climID in c(1:nClimID)){
  P0 <- PRELES(DOY=rep(1:365,maxYears),
               PAR=dat[id==climID]$PAR,
               TAir=dat[id==climID]$TAir,
               VPD=dat[id==climID]$VPD,
               Precip=dat[id==climID]$Precip,
               CO2=dat[id==climID]$CO2,
               fAPAR=rep(1,(365*maxYears)),LOGFLAG=0,p=pPRELES,
               CO2model = 2)$GPP
...
return(list(P0.dt = P0.dt, fT0.dt = fT0.dt))
}
